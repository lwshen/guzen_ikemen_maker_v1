...
**修复（第一版：按职业开放）**：把 `OCC_HAIRSTYLE` 从 `generateCharacter` 内提到模块作用域（纯字面量 IIFE，不涉及 PRNG／Date，提升是惰性的），`slotEditPool('hairStyle')` 改为「该职业专属项在前 + 通用池在后」去重合并。

**修复（第二版，用户要求「不要再做区分」，即当前实现）**：模块级算出 `HAIRSTYLE_EDIT_POOL` = 通用池 ∪ **全部**职业专属发型，`slotEditPool('hairStyle')` 无条件返回它。

- **所有职业拿到同一份 29 项列表**，不做任何职业分支；并集实际只多出 `坊主` 一项（其余职业专属发型本就在通用池内）。
- `only`／`boost` 仍然只约束**随机生成**——手动编辑不该被它锁死。实测 8 个职业（含未选职业）列表逐项一致。

**为何不改数据**：给 `pools.hairStyles` 直接加 `坊主` 会让第 5 层的常量深比出现差异（需登记为有意分歧），而且会让坊主进入**所有职业的随机生成**，改变日文基线输出。改编辑器侧则两者都不受影响。
