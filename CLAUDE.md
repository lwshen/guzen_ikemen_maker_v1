...
- 自动检查按**描述性名称**称呼，不用层号：结构检查（`verify:structure`）、行为比对（`verify:behavior`）、截图比对与存量数据检查（`verify:visual`）、数据表核对（`verify:data`）、界面残留扫描（`verify:en`）、翻译表对账（`verify:i18n`）、运行时漏翻检查（`verify:missing`）。完整对照表见 MIGRATION_VERIFICATION.md 开头的「检查体系一览」。脚本文件名沿用历史的 `layerN` 编号，仅作为文件名引用。
