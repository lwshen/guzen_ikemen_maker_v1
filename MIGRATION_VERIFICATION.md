...
**已知局限**：① 复合文本生成器（`sportsHistoryText`/`bioLine`/`underwearDesc` 等 `english` 布尔参数族）在 zh 模式回退日文输出——它们与 prompt 代码共享内部实现，扩展属 prompt 语言轴工程；② 稀有度徽章、足部轴标签等少量深层结构同样回退日文；③ 中文文案为机器起草，建议母语者过目（好在审校者就是仓库主人）。
