...
import {
  ...
  function renderProfile(){
    ...
    const ikm = ST.current.ikemenIndexMode === '表示する' ? (()=>{ const sc = ikemenScore(ST.current); return `<div class="subcard" style="margin:0 0 12px"><h3>${ST.uiLang==='en' ? 'Handsome Index' : 'イケメン指数'} <span class="pill" style="margin-left:8px">${sc} / 100 — ${ikemenRank(sc, ST.uiLang==='en')}</span></h3><div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:6px">${ikemenBreakdown(ST.current).map(([l,d])=>`<span class="pill">${l} ${d>0?'+':''}${d}</span>`).join('') || `<span class="notice">${ST.uiLang==='en' ? 'All standard features.' : 'すべて標準的な造形。'}</span>`}</div><p class="notice" style="margin:6px 0 0">${ST.uiLang==='en' ? 'Standard features score exactly 50. Computed from 19 facial axes; never written into prompts.' : '標準的な造形がちょうど50ptになる設計です（顔の19軸から算出。プロンプトには含めません）。'}</p></div>` })() : '';
    const bdHtml = ikm + `<div class="subcard" style="margin:0 0 12px"><h3>${ST.uiLang==='en' ? 'Rarity Breakdown' : 'レア内訳'} <span class="pill" style="margin-left:8px">${rr[0]} pt / ${rr[1]}</span></h3><div style="display:flex;flex-wrap:wrap;gap:4px">${bd.length ? bd.map(([l,p])=>`<span class="pill">${l} +${p}</span>`).join('') : `<span class="notice">${ST.uiLang==='en' ? 'No rare points this time.' : '今回はレア該当なし。'}</span>`}</div></div>`;
  ...
  function renderPromptTabs(){
    ...
    if(chip){
      if(ST.current){ chip.textContent = `⚡ ${ST.current.promptTarget || 'ChatGPT'} ・ ${ST.current.promptLanguage === 'English' || ST.uiLang==='en' && !ST.current.promptLanguage ? 'English' : (ST.current.promptLanguage || '日本語')}`; chip.style.display=''; }
