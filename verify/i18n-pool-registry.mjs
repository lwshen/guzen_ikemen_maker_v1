...
export const VALUE_POOLS = [
  ...
  // flow.js:731 里硬编码的穿搭笔记编辑器候选项（运行时漏翻检查上线首日
  // 收割到的漏网词库——硬编码字面量是总清单的固有盲区，靠运行时兜底发现）
  {
    name: "hardcoded-stylenote-options",
    values: () => [
      "清潔感重視のベーシックな着こなし",
      "柄物や差し色をその日の気分で",
      "モノトーン中心・機能優先",
  ...
  // 内在资料基础信息行的固定输出句（inner.js 里 chooseInnerEducation/
  // Income/Blood 的代码字面量——2026-08-14 穷举，运行时漏翻检查兜漏网）
  {
    name: "inner-basic-fixed-outputs",
    values: () => [
      "AB型",
      "AB型（Rh−）",
      "A型",
