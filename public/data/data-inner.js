...
// index.html:4577
GUZEN_DATA.INNER_DISPLAY_KEYS = ['dream','desire','weakness','talent','past','pronoun','income','origin','education','complex','blood','love'];

// index.html:4591
GUZEN_DATA.INNER_DREAMS = {
...
// index.html:4598
GUZEN_DATA.INNER_DREAM_CAT = { student:[['内定を取ること',9],['単位を落とさず卒業すること',6],['奨学金を借りずに済ませること',3]], creative:[['自分の作品で食べていくこと',6],['賞を取ること',4],['個展・単独ライブを開くこと',3]], sports:[['大会で結果を出すこと',6],['指導者として独立すること',3]], medical:[['専門資格を極めること',5],['開業すること',3]], trade:[['一人前と呼ばれること',6],['自分の店・工房を持つこと',4]], retired:[['悠々自適を貫くこと',8]] };

// index.html:4599
GUZEN_DATA.INNER_DESIRES = [
...
// index.html:4621
GUZEN_DATA.INNER_PRONOUNS_BASE = [['俺',13],['オレ',5.5],['おれ',3.5],['僕',4.5],['ボク',1],['ぼく',0.7],['私',2.2],['わたくし',0.3,'r'],['自分',2.5],['わし',0.3],['うち',0.4,'r'],['名前呼び',0.2,'r'],['場面で使い分ける（俺⇄僕⇄私）',2],['一人称がまだ確立していない',0.8]];

// index.html:4622
GUZEN_DATA.INNER_LOVE_BASE = [['女性',86],['男性',3.5,'r'],['男女どちらも',2.5,'r'],['まだ揺らいでいて分からない',2,'r'],['恋愛にあまり興味がない',3,'r'],['二次元にしか本気になれない',1,'r'],['恋愛よりも推しがすべて',0.7,'r']];

// index.html:4623
GUZEN_DATA.INNER_LOVE_NOTES = [['年上に惹かれがち',5],['年下が好み',4],['同い年が落ち着く',4],['面食い',4],['性格重視',5],['声フェチ',2.5],['手フェチ',1.5,'d'],['匂いに弱い',1.5,'d'],['うなじに弱い',1.5,'d'],['惚れっぽい',4],['冷めやすい',3],['一途',5],['依存しがち',2,'d'],['束縛強め',1.5,'d'],['尽くしすぎて重いと言われる',2,'d'],['ダメな相手ばかり好きになる',2,'d'],['既婚者に惹かれてしまう悪癖',0.6,'d'],['元カノを引きずっている',2,'d'],['押しに弱い',3],['高嶺の花ばかり狙って玉砕する',2],['笑いのツボが合う人に弱い',4],['ギャップに弱い',4],['メガネに弱い',2],['ショートカットに弱い',2.5],['ポニーテールに弱い',2],['浴衣に弱い',1.5],['母性に弱い',2,'d'],['褒められるとすぐ好きになる',3]];

// index.html:4624
GUZEN_DATA.INNER_ORIGINS = [
...
// index.html:4627
GUZEN_DATA.INNER_COMPLEX_GENERIC = [
...
// index.html:4907
GUZEN_DATA.INNER_HOBBY_GENERIC = [['サウナ通い',4],['筋トレ',5],['ランニング',4],['フットサル',2.5],['ゴルフ練習場通い',2.5],['釣り',3],['キャンプ',3],['登山',2.5],['ロードバイク',2],['自炊・料理',4],['ラーメン食べ歩き',3.5],['カレー屋巡り',2],['コーヒー（豆から淹れる）',2.5],['クラフトビール巡り',2],['日本酒の飲み比べ',2],['映画鑑賞',5],['ミニシアター通い',1.5],['読書',4],['マンガ',4],['ゲーム（FPS）',3],['ゲーム（RPG）',3],['レトロゲーム収集',1.5],['ボードゲーム会',1.5],['麻雀',2],['将棋アプリ',2],['カメラ・写真',3],['フィルムカメラ',1.5],['プラモデル',2],['ガンプラ',2],['鉄道（乗り鉄）',1.5],['御朱印集め',1.5],['城巡り',1.5],['美術館巡り',2],['水族館巡り',1.5],['観葉植物',2.5],['熱帯魚・アクアリウム',1.5],['メダカ飼育',1.2],['DIY',2.5],['車いじり',2],['バイクツーリング',2],['スニーカー収集',2],['古着屋巡り',2],['レコード収集',1.5],['ギター',2.5],['カラオケ',3],['ダーツ',1.5],['ビリヤード',1.2],['ボルダリング',1.5],['サーフィン',1.2],['スノボ（冬季）',2],['野球観戦',3],['サッカー観戦',2.5],['温泉巡り',3],['銭湯巡り',2.5],['食べ歩きブログ巡回',2],['ポイ活',1.5],['競馬（趣味の範囲）',1.5,'d'],['盆栽',1,'r'],['蕎麦打ち',0.8,'r'],['献血（趣味と言い張る）',0.8,'r']];

// index.html:4908
GUZEN_DATA.INNER_HOBBY_BY_VIBE = {'スポーツ系':[['フットサル',6],['ジム通い',6],['ランニング',5]],'古着系':[['古着屋巡り',8],['レコード収集',3]],'オタク系':[['アニメ鑑賞',7],['ゲーム（RPG）',6],['フィギュア収集',3]],'アウトドア系':[['キャンプ',8],['登山',6],['釣り',4]],'バンドマン系':[['ギター',8],['ライブ通い',5],['機材集め',4]],'レトロ系':[['純喫茶巡り',7],['フィルムカメラ',4]],'メガネ知的系':[['読書',7],['美術館巡り',4]],'おじさん系':[['銭湯巡り',6],['晩酌の肴づくり',5],['競馬（趣味の範囲）',2,'d']],'ギャル男系':[['サウナ通い',6],['クラブ・音楽イベント',4]],'ホスト系':[['筋トレ',6],['美容・スキンケア研究',5]],'サブカル系':[['ミニシアター通い',6],['レコード収集',4],['ZINE集め',2]],'清楚系':[['カフェで読書',6],['植物の世話',3]],'ヤンキー系':[['バイクいじり',7],['地元の草野球',3]],'韓国風':[['カフェ巡り',5],['ファッション研究',4]]};

// index.html:4909
GUZEN_DATA.INNER_MYBOOM_MODERN = [['朝サウナ',4],['白湯を飲むこと',3],['オートミールアレンジ',2],['二郎系ラーメン開拓',3],['韓国ドラマ一気見',2.5],['ショート動画の沼',3],['推し配信者の切り抜き巡回',2],['プロテインの味比べ',3],['コンビニ新作スイーツ評論',3.5],['スマートウォッチの睡眠スコア',2.5],['キャンプ飯動画',2.5],['サブスクの解約整理',2],['歩数計チャレンジ',2],['冷凍餃子の焼き方研究',3],['電気圧力鍋',2],['ふるさと納税の返礼品研究',2.5],['ノーカフェイン生活（3日目）',2],['AIに献立を決めてもらうこと',1.5],['マイボトル持参',2.5],['昼休みの散歩',3]];

// index.html:4910
GUZEN_DATA.INNER_MYBOOM_COMMON = [['ストレッチ',3.5],['蒙古タンメン的な激辛麺',2.5],['卵かけご飯の醤油研究',3],['納豆のトッピング研究',2.5],['缶コーヒーの飲み比べ',2.5],['靴磨き',2],['クロスワード',1.5],['青春時代の曲を聴き直すこと',3.5],['妻・彼女の真似をした健康法',2],['寝る前の白黒映画',1.2,'r'],['ぬか漬け',1.5],['ベランダ菜園のミニトマト',2.5],['ラジオの深夜放送',2],['手帳・文房具',2],['スクワット',3],['近所の野良猫の観察',2.5],['指のストレッチ',1.5],['出汁を取ること',1.5]];

// index.html:4911
GUZEN_DATA.INNER_MYBOOM_RETRO = [['ビックリマンシール集め',3],['ファミコンの攻略',3],['深夜ラジオのハガキ職人',2],['喫茶店のナポリタン巡り',3],['レンタルビデオ屋の新作チェック',3],['プロ野球ニュースの録画',2.5],['ラジカセでのエアチェック',2.5],['プラモ屋通い',2.5]];

// index.html:4912
GUZEN_DATA.INNER_FOOD_LIKE = [['唐揚げ',7],['餃子',6],['家系ラーメン',4],['味噌ラーメン',4],['豚骨ラーメン',4],['寿司（サーモン）',4],['寿司（中トロ）',3],['焼肉（ハラミ）',5],['焼肉（タン塩）',4],['カレーライス',6],['ハンバーグ',5],['オムライス',4],['親子丼',4],['カツ丼',4.5],['天ぷらそば',3],['讃岐うどん',3.5],['牛丼',5],['麻婆豆腐',4],['チャーハン',5],['ナポリタン',3.5],['生姜焼き',5],['卵かけご飯',4],['明太子',3],['刺身の盛り合わせ',3.5],['焼き鳥（皮）',3],['焼き鳥（ねぎま）',3.5],['もつ煮',3],['おでん（大根）',3],['キムチ鍋',3.5],['もつ鍋',3],['ピザ',3.5],['ステーキ',4],['回転寿司ぜんぶ',3.5],['実家の味噌汁',3.5],['白米そのもの',3.5],['プリン',2.5],['チョコミント',1.5],['大福',2],['シュークリーム',2.5],['メロンパン',2.5],['コンビニのホットスナック',3.5],['駅そば',2.5],['カツカレー',4],['エビフライ',3.5],['ぶり大根',2],['サバの味噌煮',3],['ローストビーフ',2.5],['ペペロンチーノ',2.5],['タコライス',1.5],['ジンギスカン',1.5]];

// index.html:4913
GUZEN_DATA.INNER_FOOD_HATE = [['パクチー',5],['セロリ',4.5],['春菊',3],['ゴーヤ',3.5],['レバー',4],['牡蠣',3.5],['うに',2.5],['納豆',3],['くさや',2],['ブルーチーズ',3],['酢の物',2.5],['らっきょう',3],['梅干し',2.5],['グリンピース',3],['ミニトマト',2.5],['生玉ねぎ',3],['椎茸',3.5],['なす',2.5],['ピーマン',2.5],['激辛料理',3],['甘すぎるスイーツ',2.5],['生クリーム',2.5],['あんこ',2],['ホルモン',2.5],['生もの全般',2],['マヨネーズ',2],['ドリアン',2],['八角の香り',2.5],['山菜の苦み',2],['酢豚のパイナップル',4],['パンに入ったレーズン',3.5],['わさび',2.5],['からし',2],['セロリ以上にパセリ',1.5],['グミ',1.5],['貝類全般',2],['スイカ',1.5],['メロン',1.2],['牛乳',2],['特になし（好き嫌いゼロ）',5]];

// index.html:4914
GUZEN_DATA.INNER_HEALTH_BASE = [['至って健康（風邪もめったに引かない）',7],['おおむね良好',9],['おおむね良好（肩こりだけが友達）',5],['健康だが健診の数値がじわじわ来ている',4],['健康優良（献血の常連）',1.5,'r'],['丈夫だけが取り柄',4],['季節の変わり目に弱い',3],['良好（ただし花粉の季節を除く）',4]];

// index.html:4915
GUZEN_DATA.INNER_HEALTH_MID = [['血圧がやや高め',4],['尿酸値に警告が出ている',3,'d'],['γ-GTPと戦っている',2.5,'d'],['腰と長い付き合い',4],['老眼が始まった',3],['五十肩の気配',2],['健診の再検査を先延ばし中',3,'d'],['禁煙に成功して3年目',2],['禁煙に挑戦中（3回目）',2,'d']];

// index.html:4916
GUZEN_DATA.INNER_LIVING_SINGLE = [['一人暮らし（賃貸アパート）',9],['一人暮らし（賃貸マンション）',6],['実家暮らし',7],['会社の独身寮',2],['社宅住まい',1.5],['友人とルームシェア',1.5],['祖父母の家に同居（家賃代わりに手伝い）',0.8,'r'],['住み込み（職場の上）',0.6,'r']];

// index.html:4917
GUZEN_DATA.INNER_LIVING_MARRIED = [['家族と賃貸マンション',6],['家族と持ち家（ローン返済中）',6],['家族と持ち家（実家を建て替え）',2],['単身赴任中（家族は地元）',1.2,'d'],['妻の実家の近くに新居',1.5],['二世帯住宅（親と同居）',1.5]];

// index.html:4918
GUZEN_DATA.INNER_FRIEND_MEET = [['幼なじみ',5],['小学校からの腐れ縁',4],['中学の部活仲間',4],['高校の同級生',6],['高校の部活仲間',5],['大学のサークル仲間',4],['大学の同期',3.5],['専門学校の同期',2],['職場の同期',5],['前の職場の同期',3],['バイト先で知り合った',3],['行きつけの店の常連仲間',2.5],['サウナで知り合った',1.5],['草野球チームの仲間',2],['ネトゲのフレンド（オフ会で意気投合）',1.5],['ジム仲間',2],['隣の席だっただけの縁',2.5]];

// index.html:4919
GUZEN_DATA.INNER_FRIEND_NAMES = ['ユウタ','ケンジ','ダイキ','ショウ','タクミ','ヒロ','カズ','リョウ','シンジ','マサ','コウジ','テツ','ナオ','ジュン','ゴロー','ハヤト','ワタル','アキラ'];

// index.html:4920
GUZEN_DATA.INNER_FRIEND_FREQ = [['月イチで飲む',5],['週末によく遊ぶ',3],['年2回の旅行が恒例',2],['ほぼ毎日どうでもいい連絡をし合う',3],['連絡は稀だが会えば一瞬で戻る',4],['サシ飲みできる唯一の相手',3],['家族ぐるみの付き合い',2],['月イチのフットサル仲間',2]];

// index.html:4921
GUZEN_DATA.INNER_LOVER_NONE = [['なし（出会いがない）',6],['なし（仕事が恋人）',3],['なし（半年前に破局）',2.5],['なし（絶賛片思い中）',2.5],['なし（マッチングアプリと格闘中）',2.5],['なし（気楽で気に入っている）',3],['なし（友達どまりの人はいる）',2.5]];

// index.html:4922
GUZEN_DATA.INNER_LOVER_YES = [['恋人あり（付き合って3ヶ月）',3],['恋人あり（付き合って1年）',4],['恋人あり（付き合って2年・倦怠期）',2],['恋人あり（学生時代から）',2.5],['恋人あり（遠距離）',1.5],['恋人あり（同棲中）',2.5],['恋人あり（最近ケンカ中）',1.5]];

// index.html:4923
GUZEN_DATA.INNER_MEMORY_BASE = [['修学旅行の夜の告白大会',4],['文化祭の打ち上げ',4],['卒業式で第二ボタンを聞かれなかったこと',2.5],['初めての一人旅',3.5],['免許合宿での出会いと別れ',3],['成人式の同窓会',3.5],['初任給で親に鰻をおごった日',4],['上京の日の新幹線の窓',3],['富士山頂のご来光',2],['夜行バスで見た朝焼け',2.5],['地元の花火大会',4],['祖父母の家の夏休み',4],['台風の日の停電で家族で食べたカップ麺',3],['猫を拾った雨の日',2],['初めて売上目標を達成した日',3],['深夜のファミレスで語り明かした夜',4],['文化祭のバンドでの1曲',2.5],['満員の甲子園アルプス席',1.5],['卒業旅行の朝帰り',3]];

// index.html:4924
GUZEN_DATA.INNER_JP_PREFS = [
...
// index.html:5262
GUZEN_DATA.INNER_CATS = [['basic','🪪 基本データ','🪪 Basic Data','icat-basic'],['life','🏠 暮らし・家族','🏠 Life & Family','icat-life'],['daily','☕ 日常・嗜好','☕ Daily & Tastes','icat-daily'],['mind','💭 内面','💭 Inner Self','icat-mind'],['past','🕰 過去・人間関係','🕰 Past & People','icat-past'],['adult','🌙 オトナの事情','🌙 Grown-up','icat-adult']];

// index.html:5458
GUZEN_DATA.INNER_PRINCIPLES = [
...
// index.html:5463
GUZEN_DATA.INNER_UNFORGIVABLES = [
...
// index.html:5684
GUZEN_DATA.INNER_FASHION_SENSE = [
...
// index.html:5736
GUZEN_DATA.INNER_LOVE_NOTE_ANY = [['年上に惹かれがち',5],['年下が好み',4],['同い年が落ち着く',4],['面食い',4],['性格重視',5],['声フェチ',2.5],['惚れっぽい',4],['冷めやすい',3],['一途',5],['依存しがち',2,'d'],['束縛強め',1.5,'d'],['尽くしすぎて重いと言われる',2,'d'],['ダメな相手ばかり好きになる',2,'d'],['押しに弱い',3],['高嶺の花ばかり狙って玉砕する',2],['笑いのツボが合う人に弱い',4],['ギャップに弱い',4],['メガネに弱い',2],['褒められるとすぐ好きになる',3],['連絡はマメな方',3],['連絡不精で振られがち',2.5,'d'],['理想が高いと言われる',2.5],['同じ趣味の人がいい',3.5],['年の差は気にしない',2.5],['食べっぷりのいい人に弱い',3],['敬語が可愛い人に弱い',2.5],['方言に弱い',2.5],['既婚者に惹かれてしまう悪癖',0.6,'d'],['元恋人を引きずっている',2,'d'],['匂いに弱い',1.5,'d'],['手フェチ',1.5,'d']];

// index.html:5737
GUZEN_DATA.INNER_LOVE_NOTE_F = [['ショートカットに弱い',2.5],['ポニーテールに弱い',2],['浴衣に弱い',1.5],['母性に弱い',2,'d'],['うなじに弱い',1.5,'d'],['姉御肌に弱い',2.5],['小柄な人に惹かれる',2],['背の高い人に惹かれる',2],['黒髪ロングが原点',2],['笑顔に弱い',3.5],['料理上手に弱い',3],['泣きぼくろに弱い',1.5]];

// index.html:5738
GUZEN_DATA.INNER_LOVE_NOTE_M = [['筋肉質な人に弱い',2.5],['低い声に弱い',2.5],['スーツ姿に弱い',2.5],['ヒゲが似合う人に弱い',2],['年上の包容力に弱い',2.5],['塩対応に弱い',1.5],['ジャージ姿に弱い',1.5],['手の骨っぽさに弱い',1.5,'d'],['体格差に弱い',2],['職人気質な人に惹かれる',2],['寡黙な人に惹かれる',2.5],['笑い上戸に弱い',2]];

// index.html:5739
GUZEN_DATA.INNER_LOVE_NOTE_BI = [['性別より人柄',5],['どちらかといえば女性寄り',3],['どちらかといえば男性寄り',2.5],['惹かれる相手に性別は関係ない',3.5],['その時々で揺れる',2],['好きになった人が性別',3]];

// index.html:5878
GUZEN_DATA.INNER_KANA2KANJI = {ユウタ:['悠太','ユウタ'],ケンジ:['健二','ケンジ'],ダイキ:['大輝','ダイキ'],ショウ:['翔','ショウ'],タクミ:['拓海','タクミ'],ヒロ:['宏樹','ヒロキ'],カズ:['和真','カズマ'],リョウ:['涼','リョウ'],シンジ:['慎二','シンジ'],マサ:['雅之','マサユキ'],コウジ:['浩二','コウジ'],テツ:['哲平','テッペイ'],ナオ:['直樹','ナオキ'],ジュン:['潤','ジュン'],ゴロー:['吾郎','ゴロー'],ハヤト:['隼人','ハヤト'],ワタル:['航','ワタル'],アキラ:['彰','アキラ']};

// index.html:5955
GUZEN_DATA.INNER_FIELD_GEN = {innerDream:'dream', innerDesire:'desire', weaknessMind:'weakness', weaknessBody:'weakness', innerTalent:'talent', pastUpbringing:'past', pastTrauma:'past', pronoun:'pronoun', incomeText:'income', originText:'origin', educationText:'education', complexText:'complex', bloodType:'blood', loveTarget:'love', maritalText:'marital', livingText:'living', familyText:'family', birthplaceText:'birthplace', birthdateText:'birthdate', nicknameText:'nickname', speechText:'speech', memoryText:'memory', friendText:'friend', loverText:'lover', residenceText:'residence', healthText:'health', hobbyText:'hobby', myBoomText:'myboom', foodLikeText:'foods', foodHateText:'foods', principleText:'principle', unforgivableText:'unforgivable', fuzokuText:'fuzoku', gambleText:'gamble', firstExpText:'firstexp', weekFreqText:'weekfreq', drinkText:'drink', smokeText:'smoke', loveCountText:'lovecount', assetText:'asset', selfFreqText:'selffreq', fashionSenseText:'fashionsense'};
