...
import {
  ...
  function renderProfile(){
    ...
    const sections = [
      ['basic', L.basic, [[L.name,ST.current.name,'name'],[L.age,displayValue('age',ST.current.age),'age'],[L.era, ST.uiLang==='en' ? `${ST.current.eraYear || '2026'} (${eraProfile(ST.current.eraYear || '2026').labelEn})${ST.current.season ? ` / ${displayValue('season', ST.current.season) || ST.current.season}` : ''}` : ST.uiLang==='zh' ? `${ST.current.eraYear || '2026'}年（${ERA_LABEL_ZH[eraProfile(ST.current.eraYear || '2026').labelJa] || eraProfile(ST.current.eraYear || '2026').labelJa}）${ST.current.season ? `・${displayValue('season', ST.current.season)}` : ''}` : `${eraLabel(ST.current.eraYear)}（${eraProfile(ST.current.eraYear || '2026').labelJa}）${ST.current.season ? `・${ST.current.season}` : ''}`,'eraYear'],[L.natEth,`${displayValue('nationality',ST.current.nationality)} / ${displayValue('ethnicity',ST.current.ethnicity)}`,'nationality,ethnicity'],[L.roleVibe,`${displayValue('role',ST.current.role)} / ${displayValue('vibe',ST.current.vibe)}${ST.current.occupationMode ? `（${ST.uiLang==='en' ? displayValue('occupationMode', ST.current.occupationMode) || ST.current.occupationMode : ST.current.occupationMode}）` : ''}`,'role,vibe'],[LT('経験競技スポーツ', 'Sports Background', '运动经历'), sportsHistoryText(ST.current, ST.uiLang), 'sportsHistory']].concat(ST.current.groupSetting ? [[L.group, `${displayValue('groupSetting',ST.current.groupSetting) || ST.current.groupSetting} / ${displayValue('groupPosition',ST.current.groupPosition) || ST.current.groupPosition}`]] : []).concat(ST.current.friendOf ? [[L.friendRow, friendRelationText(ST.current, ST.uiLang)]] : []).concat([[L.mbti,`<span class="mini-badge">${mbtiDisplay(ST.current)}</span>`,'mbti']])],
      ...
      ['outfit', L.outfitSection, (()=>{
        ...
        if(!ST.current.workUniform){
          ...
          rows.push([LT('靴（平日）','Work Shoes','鞋子（工作日）'), `${B(ST.current.shoesBrand)}${ST.current.shoesColor?`${ST.current.shoesColor}・`:''}${displayValue('shoes',ST.current.shoes)}`, 'shoes,shoesBrand,shoesColor']);
          if(isSuit){
        ...
        if(ST.current.holidayOutfitType){
          ...
          rows.push([LT('上着（休日）','Casual Outer','外套（休息日）'), `${ST.current.holidayOuterBrand?`${displayValue('outfitBrand',ST.current.holidayOuterBrand)}・`:''}${ST.current.holidayOuterColor?`${ST.current.holidayOuterColor}・`:''}${displayValue('holidayJacket',ST.current.holidayJacket||'指定なし')}`, 'holidayJacket,holidayOuterBrand,holidayOuterColor']);
          rows.push([LT('トップス（休日）','Casual Top','上装（休息日）'), `${ST.current.holidayTopBrand?`${displayValue('outfitBrand',ST.current.holidayTopBrand)}・`:''}${ST.current.holidayTopColor?`${ST.current.holidayTopColor}・`:''}${displayValue('holidayTop',ST.current.holidayTop||'')}`, 'holidayTop,holidayTopBrand,holidayTopColor']);
          rows.push([LT('ボトムス（休日）','Casual Bottom','下装（休息日）'), `${ST.current.holidayBottomBrand?`${displayValue('outfitBrand',ST.current.holidayBottomBrand)}・`:''}${ST.current.holidayBottomColor?`${ST.current.holidayBottomColor}・`:''}${displayValue('holidayBottom',ST.current.holidayBottom||'')}`, 'holidayBottom,holidayBottomBrand,holidayBottomColor']);
          rows.push([LT('靴（休日）','Casual Shoes','鞋子（休息日）'), `${ST.current.holidayShoesBrand?`${displayValue('outfitBrand',ST.current.holidayShoesBrand)}・`:''}${ST.current.holidayShoesColor?`${ST.current.holidayShoesColor}・`:''}${displayValue('holidayShoes',ST.current.holidayShoes||'')}`, 'holidayShoes,holidayShoesBrand,holidayShoesColor']);
