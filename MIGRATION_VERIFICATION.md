...
**最初怀疑那 77 个 `english` 布尔参数是债，查下来不是**：`pools.promptLanguages === ["日本語","English"]`，**提示词语言天生只有两种**（由 `isEnglish(c)` 驱动），**界面语言才是三种**。这是有意的双轴模型，布尔在提示词侧是正确的，不该改成三态。

也专门查了最危险的情形——有没有谁把 `ST.uiLang` 直接塞进 `english` 参数（那会让中文用户看到英文）：**0 处命中**。所以现状是「看着杂、实际自洽」。

### 10.2 清理一：消除「`'zh'` 骑在布尔参数上」

跨两轴的双职能函数原先这样调用，重复 8 次：

```js
sportsHistoryText(ST.current, ST.uiLang === "zh" ? "zh" : ST.uiLang === "en");
```

字符串 `'zh'` 是 truthy，所以**任何只认布尔的函数一旦被这样调用就会静默返回英文**。而且这个三态表达式本身是把 `ST.uiLang`（本就是 `'ja'|'en'|'zh'`）做了一次**有损重编码**。

**首轮只找到 7 个双职能函数，穷尽扫描后是 8 个**——`underwearDesc` 漏了，因为 `ui.js:88` 直接传字面量 `'zh'` 而非那个三态表达式。补扫方式：解析所有调用的**末位实参**（括号配对，非正则行匹配），列出以 `'zh'` 或含 `uiLang==='zh'` 为末参的被调函数。
...
新增 `langOf(v)`（i18n.js）把提示词侧布尔抬升进语言码域，供 3 处透传调用者使用。**最终态：每个函数只吃语言码或只吃布尔，绝不混用。**

### 10.3 清理二：`INNER_LABEL_ZH` 是唯一的例外岛

`inner.js` 里 43 行、44 处走侧表 `LT('JA','EN', INNER_LABEL_ZH['JA'])`，而全项目另外 145 处 `LT()` 都把三种语言内联在调用点。这是上一轮 codemod 批量改 44 个三元表达式的产物，不是设计。已全部折回内联，`INNER_LABEL_ZH` 的数据导出与第 5 层白名单条目一并删除（`*_ZH` 导出 9 → 8）。
...
六层全绿，L2 **123 场景 0 差异**。但 L2 只证明日／英**提示词**不变，L6 只覆盖**标签**——这 8 个函数的**中文显示**没有任何既有层覆盖。所以专门做了一次改动前后对拍（`scratchpad/snap3.mjs`）：4 个种子 × 3 语言 × 2 职业 = 24 场景，抓取全部资料行、卡槽值与提示词全文，`git stash` 前后各跑一次。
...
## 13. 第 7 层：跨语言翻译表奇偶性（2026-08-12，分支 `verify-i18n-parity`）

**动机**：第 6 层问的是「**渲染出的界面**有没有未翻译的日文」，但只能看到扫描时恰好打开的表面；历次翻译 pass（§12.3 等）都出现过「补了 zh 漏了 en」或反向的静默偏科。第 7 层改问「**翻译表本身**在语言之间是否互相完整」——纯源码级、免构建免浏览器，`npm run verify:i18n`（已加入 `npm run verify` 链，排在 L5 之后）。

**四项检查**（`verify/layer7-i18n-parity.mjs`）：

| 检查                 | 对象                                                        | 判定                                                                                                                                                                                                                                                         |
| -------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 7a uiText 深度键奇偶 | ja 为参照语言                                               | en/zh 必须含全部 ja 键路径且无孤儿键（当前 223/223）                                                                                                                                                                                                         |
| 7b 标签映射表        | slotLabelMap 等 5 表                                        | 每键必须三语齐全——`data-i18n-zh.js` 的合并用 `if (map[k])` 守卫，合并键打错字会静默丢失，此检查专抓这个                                                                                                                                                      |
| 7c 成对值表          | valueTranslations↔Zh、sceneTranslations↔Zh、FRIEND_REL/HIER | 两语言键集合的并集即「应译集合」，单边缺失即缺口。**zh 侧严格**（本次补齐最后 8 条：足元地面 6 + videoDur 2）；**en 侧棘轮**——en 表冻结于 V3.2.0 基线而 zh 包覆盖面远超，1868 条存量缺口冻结于 `verify/golden/en-i18n-gap.json`，只有**新增** en 缺口才 FAIL |
| 7d 空值              | 全部翻译表 + uiText                                         | 译文必须为非空字符串                                                                                                                                                                                                                                         |

**棘轮的门禁纪律**（变异测试审查后加固）：执行路径**永不自铸基线**——gap 文件缺失或损坏直接 FAIL（否则 CI 全新克隆会静默把当下所有缺口吸收为新基线，棘轮形同虚设）；基线只能用显式 `node verify/layer7-i18n-parity.mjs --write-baseline` 创建。**自动收紧**：某冻结键补上 en 翻译后，绿跑会当场重写基线剔除它并 FAIL 一次（提示提交收紧后的 gap 文件）——否则该键日后回退会永远躲在过期冻结集后面。

**与既有层的关系**：L5 管「基线条目不许改删」，L7 管「语言之间互相追平」，L6 管「渲染结果无残留」——三层正交。今后任何一次给某语言补翻译的 pass，若没同步补另一语言，L7 当场 FAIL（棘轮方向）或立即 FAIL（zh 方向）。

**验证的验证**：交付前以 8 个种子变异做了变异测试（删 uiText 键、孤儿键、合并表键打错字、新增单边 zh 值、删 zh 值、置空译文、干净对照、棘轮收紧路径），8/8 均被正确判定；对抗性审查发现的「自铸基线」「不自动收紧」「keyPaths 空子树盲区」均已修复。
