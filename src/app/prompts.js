...
import {
  ...
  underwearDesc, underwearShapeGuide, uniformHatPhrase, uniformJacketPhrase, salienceTop, generateCoordinatedOutfit, chooseSmileTraits, buildBioHook2, buildBioLine2, buildPersonSummary, bmiLine, natUniformFix,
} from './generate.js';
import {
  LT,
  ...
  displayValue, mbtiDescription, outfitStyleGuide, promptTargetGuide, promptValue, slotLabel, suggestCardRarity, langOf,
} from './i18n.js';
import {
...
import {
  ...
  function rerollProfile(key){
    ...
    if(key==='holidaySockType'){ const co=generateCoordinatedOutfit(ST.current.holidayOutfitType||ST.current.outfitType, ST.current.age, false, ST.current.nationality, ST.current.vibe, ST.current.eraYear, ST.current.season, ST.current.mbti, {senseText:ST.current.fashionSenseText, incomeText:ST.current.incomeText, assetText:ST.current.assetText, gambleText:ST.current.gambleText, role:ST.current.role}); ST.current.holidaySockBrand=co.sockBrand; ST.current.holidaySockType=co.sockType; ST.current.holidaySockColor=co.sockColor; ST.current.holidaySockUse=co.sockUse; renderAll(); return; }
    ...
    if(key==='eyebrowGroom'){ ST.current.eyebrowGroom = weighted([['自然なまま',5],['整えた形',3],['きっちりライン取り',1],['剃り込み跡あり',0.5]]); renderAll(); return; }
    if(key==='eyebrowGap'){ ST.current.eyebrowGap = weighted([['眉間は近め',1.5],['標準的な眉間',5],['眉間は離れ気味',1.5]]); renderAll(); return; }
    if(['smileEyes','smileStyle','cheekSmile','mouthCorner'].includes(key)){ ST.current[key] = chooseSmileTraits(ST.current)[key]; renderAll(); return; }
    if(key==='trainingLevel'){ ST.current.trainingLevel = chooseTrainingLevel(ST.current); applyMuscleFashion(ST.current); ST.current.bioText = buildBioHook2(ST.current); renderAll(); return; }
    if(key==='catchText'){ ST.current.catchText = buildBioHook2(ST.current); renderAll(); return; }
    if(key==='bioText'){ ST.current.bioText = buildBioLine2(ST.current) || bioLine(ST.current,false); ST.current.summaryText = buildPersonSummary(ST.current); renderAll(); return; }
...
Preserve the character's identity. Era setting: ${c.eraYear || '2026'} CE. ${eraStyleNote(c, true)} He is ${c.age} years old, ${c.nationality}, ${c.ethnicity}. His vibe is ${c.vibe}. His personality and demeanor feel ${mbtiDescription(c.mbti, 'en')}. ${facePresetPhrase(c, true)}And his age appearance is ${c.ageAppearance}. Height ${c.height}, weight ${c.weight}, body type ${bodyTypeDesc(c.bodyType, true)}${bmiLine(c,true)}. Hair is ${c.hairColor} ${c.hairStyle}. Facial hair: ${c.facialHair}. ${bodyEn}

${mode==='holiday'
  ? `[Casual outfit] This is his personal, off-duty style that reflects his personality and vibe. Base the outfit on ${c.holidayOutfitBrand || c.outfitBrand} ${c.holidayOutfitType || c.outfitType}. Outerwear: ${c.holidayOuterBrand?`${c.holidayOuterBrand} `:''}${c.holidayJacket || 'none'}. Top: ${c.holidayTopBrand?`${c.holidayTopBrand} `:''}${c.holidayTopColor?`${c.holidayTopColor} `:''}${c.holidayTop || c.top}. Bottom: ${c.holidayBottomBrand?`${c.holidayBottomBrand} `:''}${c.holidayBottomColor?`${c.holidayBottomColor} `:''}${c.holidayBottom || c.bottom}. Shoes: ${c.holidayShoesBrand?`${c.holidayShoesBrand} `:''}${c.holidayShoes || c.shoes}. Socks: ${c.holidaySockBrand || c.sockBrand} ${c.holidaySockType || c.sockType} (${c.holidaySockColor || c.sockColor}).${c.holidayEraFashionNote?` Overall silhouette: ${c.holidayEraFashionNote}.`:''}${accText(c,true,true)}${innerCasualNotes(c, true)}`
  : c.workUniform ? `[Work uniform — as a ${promptValue(c.role) || c.role}] He wears ${natUniformFix(c, c.workUniformEn, true)}. Depict it as practical issued workwear. Do NOT accurately reproduce insignia or logos of any real organization, unit, or company — keep it a generic, standard uniform style for his occupation. Outerwear: ${c.jacket && c.jacket !== 'なし' ? c.jacket : 'none'}. Top: ${c.top}. Bottom: ${c.bottom}. Shoes: ${c.shoes}${uniformHatPhrase(c, true)}. Socks: ${c.sockBrand} ${c.sockType} (${c.sockColor}, ${c.sockUse}).${accText(c,false,true)}` : `[Work outfit${c.role ? ` — as a ${promptValue(c.role)}` : ''}] Base the outfit on ${c.outfitBrand?`${c.outfitBrand} `:''}${c.outfitType}. Outerwear: ${c.jacket}. Top: ${c.topBrand?`${c.topBrand} `:''}${c.top}. Bottom: ${c.bottomBrand?`${c.bottomBrand} `:''}${c.bottom}. Shoes: ${c.shoesBrand?`${c.shoesBrand} `:''}${c.shoes}. Socks: ${c.sockBrand} ${c.sockType}; shape: ${c.sockShape}; material: ${c.sockMaterial}; color/pattern: ${c.sockColor}; condition: ${c.sockUse}.${c.tie?` Tie: ${c.tie}.`:''}${c.coat?` Coat: ${c.coat}.`:''}${c.suitSilhouette?` Suit silhouette: ${c.suitSilhouette}.`:''}${c.workFitNote?` ${c.workFitNote}.`:''}${accText(c,false,true)}`}
...
人物の特徴は維持する。時代設定は${eraLabel(c.eraYear)}頃。${countryLine(c, false)}${seasonLine(c, false)}${eraStyleNote(c, false)}${c.age}歳、${c.nationality}、${c.ethnicity}。雰囲気は${c.vibe}。性格・立ち居振る舞いの雰囲気は${mbtiDescription(c.mbti,'ja')}。${c.holidayPersona ? `平日は${c.role}として堅実に働いているが、休日は${c.vibe}の雰囲気に一変するタイプであり、この画像は休日の姿として描く。` : ''}${facePresetPhrase(c)}年齢感は${c.ageAppearance}。${realismSpec(c, false)}身長${c.height}、体重${c.weight}、体型は${bodyTypeDesc(c.bodyType, false)}${bmiLine(c,false)}。${physiqueSpec(c, false)}${heightContrastCue(c, false)}${muscleLine(c, false, true)}髪は${c.hairColor}の${c.hairStyle}。ひげは${c.facialHair}。${bodyJa}

${mode==='holiday'
  ? `【私服コーデ】仕事とは違う、本人の性格や雰囲気が表れる私服として描く。服装は${c.holidayOutfitBrand || c.outfitBrand}の${c.holidayOutfitType || c.outfitType}を基調にする。上着は${c.holidayOuterBrand?`${c.holidayOuterBrand}の`:''}${c.holidayJacket || 'なし'}、トップスは${c.holidayTopBrand?`${c.holidayTopBrand}の`:''}${c.holidayTopColor?`${c.holidayTopColor}の`:''}${c.holidayTop || c.top}、ボトムスは${c.holidayBottomBrand?`${c.holidayBottomBrand}の`:''}${c.holidayBottomColor?`${c.holidayBottomColor}の`:''}${c.holidayBottom || c.bottom}、靴は${c.holidayShoesBrand?`${c.holidayShoesBrand}の`:''}${c.holidayShoesColor?`${c.holidayShoesColor}の`:''}${c.holidayShoes || c.shoes}。靴下は${c.holidaySockBrand || c.sockBrand}の${c.holidaySockType || c.sockType}（${c.holidaySockColor || c.sockColor}）。${c.holidayEraFashionNote?`全体は${c.holidayEraFashionNote}。`:''}${accText(c,true,false)}${innerCasualNotes(c)}`
  : c.workUniform ? `【職業コーデ（${c.role}の勤務服）】服装は${c.workUniform}。支給品らしい実用的な着こなしで描く。実在の特定組織・部隊・企業の記章やロゴを正確に再現せず、標準的な${c.role}の制服スタイルにとどめる。上着は${c.jacket && c.jacket !== 'なし' ? c.jacket : 'なし'}、トップスは${c.top}、ボトムスは${c.bottom}、靴は${c.shoes}${uniformHatPhrase(c, false)}。靴下は${c.sockBrand}の${c.sockType}（${c.sockColor}、${c.sockUse}）。${accText(c,false,false)}` : `【職業コーデ${c.role ? `（${c.role}の仕事着・仕事帰りの装い）` : ''}】服装は${c.outfitBrand?`${c.outfitBrand}の`:''}${c.outfitType}を基調にする。上着は${c.jacket}、トップスは${c.topBrand?`${c.topBrand}の`:''}${c.top}、ボトムスは${c.bottomBrand?`${c.bottomBrand}の`:''}${c.bottom}、靴は${c.shoesBrand?`${c.shoesBrand}の`:''}${c.shoesColor?`${c.shoesColor}の`:''}${c.shoes}。靴下は${c.sockBrand}の${c.sockType}で、形状は${c.sockShape}、素材は${c.sockMaterial}、色・柄は${c.sockColor}、使用感は${c.sockUse}。${c.tie?`ネクタイは${c.tie}。`:''}${c.coat?`コートは${c.outerBrand?`${c.outerBrand}の`:''}${c.coat}。`:''}${c.suitSilhouette?`スーツのシルエットは「${c.suitSilhouette}」。`:''}${c.workFitNote?`${c.workFitNote}。`:''}${accText(c,false,false)}`}
...
Scene: "${scene}". Era setting: ${c.eraYear || '2026'} CE. ${eraStyleNote(c, true)} He is ${c.age} years old, ${c.nationality}, ${c.ethnicity}. His occupation is ${c.role}, his vibe is ${c.vibe}, and his MBTI is ${c.mbti} (${mbtiDescription(c.mbti,'en')}). ${facePresetPhrase(c, true)}And his age appearance is ${c.ageAppearance}. Height ${c.height}, weight ${c.weight}, body type ${bodyTypeDesc(c.bodyType, true)}${bmiLine(c,true)}. Hair is ${c.hairColor} ${c.hairStyle}. Facial hair: ${c.facialHair}. ${bodyEn}
...
He wears ${c.occupationMode==='休日' && c.holidayOutfitType ? `his casual outfit (${c.holidayOutfitBrand?`${c.holidayOutfitBrand} `:''}${c.holidayOutfitType}: top ${c.holidayTop}, bottom ${c.holidayBottom}, shoes ${c.holidayShoes})` : `his work outfit (${c.workUniform ? c.workUniformEn : `${c.outfitBrand?`${c.outfitBrand} `:''}${c.outfitType}`}: top ${c.top}, bottom ${c.bottom}, shoes ${c.shoes})`}. Keep it non-sexual and candid, with a natural standing, walking, or relaxed posture. Preserve the same face, body proportions, height impression, and hairstyle as the same character.
...
服装は${c.occupationMode==='休日' && c.holidayOutfitType ? `私服コーデ（${c.holidayOutfitBrand?`${c.holidayOutfitBrand}の`:''}${c.holidayOutfitType}、トップスは${c.holidayTop}、ボトムスは${c.holidayBottom}、靴は${c.holidayShoes}）` : `職業コーデ（${c.workUniform ? c.workUniform : `${c.outfitBrand?`${c.outfitBrand}の`:''}${c.outfitType}`}、トップスは${c.top}、ボトムスは${c.bottom}、靴は${c.shoes}）`}を着用する。作り込みすぎない日常スナップ風。非性的で、自然な立ち姿、歩き姿、たたずまいを描写する。顔立ち、体型、身長感、髪型は同一人物として保つ。
...
Era setting: ${c.eraYear || '2026'} CE. ${eraStyleNote(c, true)} Keep the character consistent with the main profile. He is ${c.age} years old, ${c.nationality}, ${c.ethnicity}. Occupation: ${c.role}. Vibe: ${c.vibe}. His personality and demeanor feel ${mbtiDescription(c.mbti,'en')}. ${c.holidayPersona ? `On weekdays he works earnestly as a ${promptValue(c.role)}, but on days off his whole vibe transforms into a ${promptValue(c.vibe)} style — depict him here in his day-off persona.` : ''} ${facePresetPhrase(c, true)}${realismSpec(c, true)} Body type: ${bodyTypeDesc(c.bodyType, true)}${bmiLine(c,true)}. Height: ${c.height}. Weight: ${c.weight}. ${physiqueSpec(c, true)}${heightContrastCue(c, true)}${muscleLine(c, true, true)} Foot size: ${c.footSize}. Facial hair: ${c.facialHair}${c.facialHair!=='なし' && ['白髪まじり','ロマンスグレー','ごま塩頭','ほぼ白髪'].includes(c.hairColor) ? ' (with graying facial hair)' : ''}. ${c.glasses && c.glasses!=='なし' ? `He wears ${promptValue(c.glasses)}. ` : ''}Hair: ${c.hairColor} ${c.hairStyle}. ${body}

${wear}
${pose}

${cardInst}
Use rarity ${rarity} and decorative effect ${promptValue(effect)}. Make the character pose catchy and memorable based on his profile, not a plain standing pose. The card should look visually polished like a premium collectible character card, with clear hierarchy, readable profile panels, strong border design, and the original logo "GuzenIkemenMakerCARD". Do not imitate a real card franchise or use copyrighted card layouts.
...
[BASICS & ERA] Photo year: ${c.eraYear || '2026'} CE. Born: ${(Number(c.eraYear)||2026)-(Number(c.age)||25)} (currently ${c.age}). ${eraStyleNote(c, true)} He is ${c.age} years old, ${c.nationality}, ${c.ethnicity}. His occupation is ${c.role}. His overall vibe is ${c.vibe}. His personality and demeanor feel ${mbtiDescription(c.mbti,'en')}. ${c.holidayPersona ? `On weekdays he works earnestly as a ${promptValue(c.role)}, but on days off his whole vibe transforms into a ${promptValue(c.vibe)} style — depict him here in his day-off persona.` : ''}${sportsHistoryLine(c, true)}
[FACE] ${facePresetPhrase(c, true)}And his age appearance is ${c.ageAppearance}. His face line is ${c.faceLine}. ${eyeAreaLine(c, true)} Tear bags: ${c.tearBags}. Nose: ${c.nose}. Base expression: ${c.mouth}.${smileLine(c,true)} Lips: ${promptValue(c.lips || '標準的な厚さの唇')}. Mouth placement: ${promptValue(c.mouthPos || '標準的な位置・大きさの口')}. Feature spacing: ${promptValue(c.faceSpacing || '標準的な配置')}. Feature proportions: ${promptValue(c.faceRatio || '標準的なバランスの比率')}. Facial symmetry: ${promptValue(c.faceAsym || 'ほぼ対称（ごく自然な左右差）')}.${realismSpec(c, true)} ${teethLine(c, true)}${faceExtraLine(c, true)} Skin: ${valueTranslations[c.skin] || c.skin}.${skinDetailLine(c, true)} Facial hair: ${c.facialHair}${c.facialHair!=='なし' ? ` (${promptValue(c.facialHairGroom || '自然に整えている')})` : ''}${c.facialHair!=='なし' && ['白髪まじり','ロマンスグレー','ごま塩頭','ほぼ白髪'].includes(c.hairColor) ? ' (with graying facial hair)' : ''}. ${c.glasses && c.glasses!=='なし' ? `He wears ${promptValue(c.glasses)}. ` : ''}Hair: ${c.hairColor} ${c.hairStyle}, ${promptValue(c.hairTexture || '直毛')}.${hairDetailLine(c, true)}
[BODY HAIR] ${bodyEn}

[PHYSIQUE & FEET] Height ${c.height}, weight ${c.weight}, body type ${bodyTypeDesc(c.bodyType, true)}${bmiLine(c,true)}. ${physiqueSpec(c, true, true)} Hip shape: ${promptValue(c.hipShape || '標準的な丸みの臀部')} (a neutral body-reference note; never emphasized or staged).${muscleLine(c, true)}${trainingLine(c, true)}${bodyRealismLine(c, true)} Foot size ${c.footSize}, foot shape ${c.footShape}; ${footWidthDesc(c, true)}.${footFeatureLine(c, true, true)}${soleDetailLine(c, true)} Use a natural standing pose that makes the balance of height, weight, and foot size easy to understand.
...
Era setting: ${b.eraYear || '2026'} CE. Season: ${promptValue(b.season) || ''}. Scene: ${scene}. Background: ${b.background}.
