...
- `src/pages/index.astro` — Astro 页面（Phase 1：逐字节原样搬运）。
...
```bash
python3 verify/phase1-extract.py && node verify/phase2-extract-data.mjs && npm run build
```

⚠️ 注意事项：

- `index.html`、`public/app.js`、`src/pages/index.astro`、`public/data/` 在 `.prettierignore` 中，不要格式化（10KB+ 单行数据表、依赖声明顺序的同名函数覆盖，见 MIGRATION_VERIFICATION.md 风险 #4/#8）。
